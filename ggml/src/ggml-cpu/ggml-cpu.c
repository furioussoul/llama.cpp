...
static void ggml_compute_forward_get_rows_q(
    ...
    const int64_t nc = ne00; // 3584
    const int64_t nr = ggml_nelements(src1); // 1，代表要处理src0的行数

    const enum ggml_type type = src0->type; // 每行的数据类型，GGMl_TYPE_Q4_K
    ggml_to_float_t const dequantize_row_q = ggml_get_type_traits(type)->to_float; // 解量化函数，int4 -> float

    assert(ne0  == nc); // 确保dst的列数=src0的列数
    assert(ne02 == ne11); // 确保src0的block数=src1的行数
    assert(nb00 == ggml_type_size(type));
    assert(ggml_nrows(dst) == nr); // nr = 1，确保要处理的src0行数=src1元素数量=dst的行数

    const int ith = params->ith; // 1
    const int nth = params->nth; // 1

    // rows per thread
    const int dr = (nr + nth - 1)/nth; // dr是每个线程需要处理的行数。
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    // extra_buffer op? 这里是尝试调用cpu硬件加速的op，如果调用成功，就直接返回，如果调用失败，就继续往下走。
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        if (state->ith == 0 && cplan->abort_callback &&
            ...
            atomic_store_explicit(&tp->abort, node_n + 1, memory_order_relaxed);//&tp->abort=-1，node_n+1；到下个节点就退出。
