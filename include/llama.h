...
extern "C" {
    ...
    // - The output string is always null-terminated(指的是字符串的末尾由一个特殊的字符 \0（空字符，ASCII 值为 0）标记。) and cleared on failure
    ...
    // p0 < 0     : [0,  p1]， 当 p0 小于 0 时，复制操作应该从位置 0 开始，直到位置 p1。
    // p1 < 0     : [p0, inf)，当 p1 小于 0 时，表示复制从 p0 开始一直到序列的结尾（无穷大 inf 表示没有明确的结束位置）。
